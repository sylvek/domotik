...
public class EchoVerticle extends AbstractVerticle {
  ...
  public void start() {
    ...
    getVertx().eventBus().consumer("trigger", message -> System.out.println("[" + LocalTime.now(ZoneId.of("Europe/Paris")) + "] " + message.headers().get("topic") + ": " + message.body() + ", [retain: " + message.headers().get("retain") + "]"));
